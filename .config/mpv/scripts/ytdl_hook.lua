...
local function is_whitelisted(url)
	...
	for match in o.include:gmatch("%|?([^|]+)") do
		if url:find(match) then
			msg.verbose("URL matches included substring " .. match .. ". Trying ytdl first.")
			return true
		end
	end
...
local function parse_yt_playlist(url, json)
	...
	if not json.extractor or (json.extractor ~= "youtube:tab" and json.extractor ~= "youtube:playlist") then
		return nil
	end
	...
	if protocol == "http_dash_segments" and not is_live then
		...
		for i = offset, #fragments do
			if not fragments[i].duration then
				msg.verbose("EDL doesn't support fragments " .. "without duration with MP4 DASH")
				return nil
			end
		end
	end
...
local function formats_to_edl(json, formats, use_all_formats)
	...
	for _, track in ipairs(formats) do
		tbr_only = tbr_only and track["tbr"] and not track["abr"] and not track["vbr"]
	end
	...
	for index = #formats, 1, -1 do
		...
		local edl_track = edl_track_joined(track.fragments, track.protocol, json.is_live, track.fragment_base_url)
		...
		if (o.force_all_formats or track.acodec) and track.acodec ~= "none" then
			tracks[#tracks + 1] = {
				media_type = "audio",
				codec = map_codec_to_mpv(track.acodec) or ext_map[track.ext],
			}
			if is_default then
		...
		if use_all_formats then
			for _, sub in ipairs(tracks) do
				...
				if sub.media_type == "video" then
					props = props
						.. ",w="
						.. as_integer(track.width)
						.. ",h="
						.. as_integer(track.height)
						.. ",fps="
						.. as_integer(track.fps)
				elseif sub.media_type == "audio" then
					props = props .. ",samplerate=" .. as_integer(track.asr)
				end
				hdr[#hdr + 1] = "!delay_open,media_type="
					.. sub.media_type
					.. ",codec="
					.. (sub.codec or "null")
					.. props
...
local function add_single_video(json)
	...
	local http_headers = has_requested_formats and requested_formats[1].http_headers or json.http_headers
	local cookies = has_requested_formats and requested_formats[1].cookies or json.cookies

	if o.use_manifests and valid_manifest(json) then
		-- prefer manifest_url if present
		format_info = "manifest"

		local mpd_url = requested_formats and requested_formats[1]["manifest_url"] or json["manifest_url"]
		if not mpd_url then
		...
		if requested_formats then
			for _, track in pairs(requested_formats) do
				max_bitrate = (track.tbr and track.tbr > max_bitrate) and track.tbr or max_bitrate
			end
	...
	if streamurl == "" then
		...
		if (all_formats and o.all_formats) and (has_requested_formats or o.force_all_formats) then
	...
	if streamurl == "" and json.url then
		...
		local edl_track = edl_track_joined(json.fragments, json.protocol, json.is_live, json.fragment_base_url)
	...
	if json.requested_subtitles ~= nil then
		...
		for _, e in ipairs(subs) do
			...
			elseif sub_info.url ~= nil and url_is_safe(sub_info.url) then
				sub = sub_info.url
			end
		...
		for i = #json.thumbnails, 1, -1 do
			...
			if thumb_info.url ~= nil then
				if o.thumbnails == "all" then
					...
					thumb_height = 0
				elseif
					(thumb_preference ~= nil and (thumb_info.preference or -math.huge) > thumb_preference)
					or (thumb_preference == nil and (thumb_info.height or 0) > thumb_height)
				then
	...
	-- set start time
	if
		(json.start_time or json.section_start)
		and not option_was_set("start")
		and not option_was_set_locally("start")
	then
	...
	-- set end time
	if (json.end_time or json.section_end) and not option_was_set("end") and not option_was_set_locally("end") then
	...
	if json.stretched_ratio ~= nil and not option_was_set("video-aspect-override") then
...
local function run_ytdl_hook(url)
	...
	elseif json["_type"] == "playlist" or json["_type"] == "multi_video" then
		...
		local self_redirecting_url = json.entries[1]["_type"] ~= "url_transparent"
			and json.entries[1]["webpage_url"]
			and json.entries[1]["webpage_url"] == json["webpage_url"]

		-- some funky guessing to detect multi-arc videos
		if
			self_redirecting_url
			and #json.entries > 1
			and json.entries[1].protocol == "m3u8_native"
			and json.entries[1].url
		then
			...
			if entry_wsubs ~= nil and json.entries[entry_wsubs].duration ~= nil then
				for j, req in pairs(json.entries[entry_wsubs].requested_subtitles) do
					local subfile = "edl://"
					for _, entry in pairs(json.entries) do
						if
							entry.requested_subtitles ~= nil
							and entry.requested_subtitles[j] ~= nil
							and url_is_safe(entry.requested_subtitles[j].url)
						then
