...
Item {
    ...
    readonly property real restW: 60 * s
    readonly property real restH: 1 * s
    ...
    readonly property size targetSize: {
        ...
        return f ? f() : Qt.size(Math.max(restW, restPips.implicitWidth + 36 * s), restH);
    }
    ...
    /**
     * Rest anchor for Ame: the pips cluster centre. The idle outline condenses
     * into the bead here before it moves.
     */
    readonly property point wakePoint: {
        void pill.width;
        void pill.height;
        return restPips.mapToItem(pill, restPips.width / 2, restPips.height / 2);
    }
